...
CONTAINS
  ...
  SUBROUTINE sub_load_gemlite_config()
    ...
    ! INTEGRATION CONTROL
    READ(unit=in,fmt='(1X)')
    READ(unit=in,fmt=*) par_gemlite_dtyr_nonacc
    READ(unit=in,fmt=*) par_gemlite_dtyr_acc
    READ(unit=in,fmt=*) par_gemlite_stp
    READ(unit=in,fmt=*) par_gemlite_sigstp
    ! I/O
    READ(unit=in,fmt='(1X)')
    READ(unit=in,fmt=*) par_gemlite_diagstp
    READ(unit=in,fmt='(L1)') opt_gemlite(iopt_gemlite_debug)
     ! close file pipe
    CLOSE(unit=in)
